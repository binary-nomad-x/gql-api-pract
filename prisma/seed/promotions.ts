...
export async function seedPromotions(
  ...
  const couponIds = generateIds(COUPON_CODES.length);
  await bulkInsert(ctx.pool, "coupons", COUPON_CODES.map((code, i) => ({
    id: couponIds[i],
    code,
  ...
  const shipIds = generateIds(shipped.length);
  await bulkInsert(ctx.pool, "shipments", shipped.map((o, i) => ({
    id: shipIds[i],
    orderId: o.id,
