...
export const Query = {
  myConversations: (_parent: unknown, _args: unknown, ctx: Context) =>
    ctx.services.conversation.getMyConversations(ctx.userId),

  conversation: (_parent: unknown, { id }: { id: string }, ctx: Context) =>
    ctx.services.conversation.getConversation(ctx.userId, id),

  messages: (
    _parent: unknown,
    args: { conversationId: string } & PaginationArgs,
    ctx: Context,
  ) => ctx.services.conversation.getConversationMessages(ctx.userId, args.conversationId, args.limit, args.offset),
};
...
export const Mutation = {
  startConversation: (
    ...
    ctx: Context,
  ) => ctx.services.conversation.startConversation(ctx.userId, userId, title),

  sendMessage: (
    _parent: unknown,
    { conversationId, content }: { conversationId: string; content: string },
    ctx: Context,
  ) => ctx.services.conversation.sendMessage(ctx.userId, conversationId, content),

  markConversationRead: (
    _parent: unknown,
    { conversationId }: { conversationId: string },
    ctx: Context,
  ) => ctx.services.conversation.markConversationRead(ctx.userId, conversationId),
};
