...
export const Coupon = {
  orders: (parent: CouponModel, _args: unknown, ctx: Context) => ctx.services.coupon.resolveCouponOrders(parent.id),
};

export const Query = {
  couponByCode: (_parent: unknown, { code }: { code: string }, ctx: Context) => ctx.services.coupon.getCouponByCode(code),
};
